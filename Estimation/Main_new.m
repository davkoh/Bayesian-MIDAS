...
beg_s = 'Jun-1997';    %%% put in first quarter of estimation as "last day - last month of quarter (MMM) - year (YYYY)" 
end_s = 'Jun-2023';    %%% put in last quarter of estimation as "last day - last month of quarter (MMM) - year (YYYY)" 

beg_eval_per = 'Mar-2007';  % specify quarter in which to begin evaluation period "last day - last month of quarter (MMM) - year" 
...
%hard_dummy = [zeros(1,size(Vars,1)*sur), ones(1,size(Vara,1)*act), ...
 %   ones(1,size(Varl,1)*lab), ones(1,size(Varmt,1)*mort)];              % dummy for hard indicators
...
%%%% IS LINE 101 OVERWRITING CHOICE IN LINE 99??
...
%Please note, that here it is assumed that the final data publication refers to the quarterly variable coming out.
...
mod_name = "mymodel5";  %%% IS THIS NEEDED? WON'T OUTPUT NAME DEFINE THE MODEL?
...
nowcast_data = struct('betas_final',betas_final, 'out', out , 'sv_obs_type', sv_obs_type, 'almonrest',almonrest, 'Xm',Xm , 'grp_idx', grp_idx,'poly',poly,'tperiod',tperiod, 'xind',xind, 'v',v, 'tail_type',tail_type, 'trend_type',trend_type,'tin',tin,'MCMC',MCMC,'yf',yf,'crpsv',crpsv,'predv',predv,'midas_prior',midas_prior,'cycpredv',cycpredv, ...
    'trendv',trendv,'svv',svv,'sv_trendv',sv_trendv);
...
saveModelOutput(output, ...
                ...
                trend_type, tail_type, midas_type);
...
% RMSE and CRPS graph
model_files = { ...
    'Output/TREND_fixed_SV_OBS_fixed_SV_PRIOR_gigg_GIGGTYPE_hier_ag_TAIL_normal_TRANSFORM_almon/results.mat', ...
};

nowcast_eval_function_single(model_files);

% Heatmap
model_file = 'Output/TREND_PC_OBS_PC_PRIOR_gigg_GIGGTYPE_hier_ag_TAIL_normal_TRANSFORM_almon/results.mat';
plot_heatmap_function(model_file);

% Trend Decomposition
plot_trend_decomposition(model_file,[1 12 19])
