...
for j = 1:(MCMC)
    ...
    % Sample g_{t+1}
     ...
     if t == 1
         t_cont = trnd(nuout(j));%sqrt((1./gamrnd(nuout(j,1)/2,2/nuout(j,1),1,1)));
     end
     if SV == 1
...
modname = strcat('output_testcorrected_wrongtrend_ag','_',num2str(hyperpars(gg,1)),'_',num2str(hyperpars(gg,2)),modelname3,modelname4,modelname5,".mat");
