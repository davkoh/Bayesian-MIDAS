...
for candidate = ms_candidates'
    ...
    if size(candidate_output.resid_all,1) ~= size(output.resid_all,1) || ...
            size(candidate_output.resid_all,2) ~= size(output.resid_all,2)
        warning('MS benchmark %s has incompatible resid_all size [%d x %d], current output is [%d x %d]. Skipping this benchmark.', ...
            char(candidate), size(candidate_output.resid_all,1), size(candidate_output.resid_all,2), ...
            size(output.resid_all,1), size(output.resid_all,2));
        continue;
    end
...
% Compute scores for MS benchmark (if available)
...
% Build results table. Always export absolute stats; add benchmark
% comparisons only when a compatible benchmark is available.
nrows = nsub;
...
if isempty(ms_output)
    T = table(row_model, row_sub, abs_rmsfe, abs_crps, ...
              'VariableNames', {'Model','Subsample','RMSFE','CRPS'});
    excel_file = fullfile(foldername, sprintf('eval_results_abs_%s.xlsx', datestr(now,'yyyy_mm_dd')));
    fprintf('\nWriting absolute results to:\n  %s\n', excel_file);
    writetable(T, excel_file, 'Sheet', 'absolute');
    fprintf('Done. Exported absolute RMSFE and CRPS only (no compatible MS benchmark).\n');
else
    ...
    fprintf('\nWriting benchmark comparison results to:\n  %s\n', excel_file);
