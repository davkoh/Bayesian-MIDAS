...
%% 2) Nowcast calendar definitions: End date Date, Start date and number of forecast periods choice
dqend = d_q(end-1);
dqstart = d_q(1);
mstart = eomdate( dqstart - calmonths(input.mlags-1)); % Adjust Monthly series for lags
...
d_q0 = d_q;
y_q = y_q(find(dqstart == d_q)+1:find(dqend == d_q)+1,: ); %% This didn't have plus 1 before::::: DAvid
...
y_m0 = y_m;
d_m0=d_m;
y_m = y_m(find(mstart == d_m)-1:find(dqend == d_m)-1,:); %% This didn't have plus 1 before::::: David
d_m = d_m(find(mstart == d_m):find(dqend == d_m)); 

% Start: Get availability of latest data for nowcasting conditional on availability
avail_ind = ~isnan(y_m(find(dqend == d_m)-input.mlags+1:find(dqend == d_m),:));
...
avail_ind_temp = nan(1,input.mlags*size(y_m,2));

for i = 1:size(y_m,2)
    avail_ind_temp(i*input.mlags-input.mlags+1:i*input.mlags) = avail_ind(i);
end
