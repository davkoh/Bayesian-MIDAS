...
data_m = data_monthly(2:end,data_incl_m+1);
transf = data_monthly(1,data_incl_m+1);        %transforms for monthly data defined directly in excel
[T,n] = size(data_m);
varnames = names_m(1,[data_incl_m+1]);
d_m = datetime(data_monthly(2:end-1,1),"ConvertFrom","datenum")+datenum(DateString,formatIn)-data_monthly(2,1); % date names


data_q = data_quarterly(:,data_incl_q+1);
[Tq,n_q] = size(data_q);
varnames_q = names_q(1,[data_incl_q+1]);
d_q = datetime(data_quarterly(2:end,1),"ConvertFrom","datenum")+datenum(DateString,formatIn)-data_monthly(2,1);
