...
function out = gigg_hier_bsts_sv_t_aut(input)
...
for j = 1:G
    ...
    p(j) = sample_ag_slice(p(j),gamma_sq(j),0.0001,1,1,2); % Samples the hierarchical a_g component 
    ...
    gamma_sq(j) = 1/gigrnd(grp_size(j)/2-p(j),stable_psi, 2, 1); 
