function plot_trend_decomposition(model_file, periods)
...
trends = get_trends(mod, periods, y);
SV = get_SV(mod, periods);
SV_Trend = get_SV_Trend(mod, periods);
cyclical = get_cyclical(mod, periods, y);
...
% Save figure
modname = fullfile(figures_dir, strcat(mod.mod_name, '_', 'trend_decomposition.pdf'));
...
function create_subplot(fig, xData, structure, y, titleText, subplot_pos, indices, xlim_range, xticks_range, use_ylim, ylim_range, include_y)
    ...
    title(hAx, titleText, 'FontSize', 16);
    set(hAx, 'FontSize', 15);
