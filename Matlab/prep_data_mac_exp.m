%% 1) Upload the data
%[data_quarterly, names_q]= xlsread(dat_file,'QuarterlyData','A4:e500');
%[data_monthly, names_m]= xlsread(dat_file,'MonthlyData','A4:aq2000');

data_quarterly = readtable(dat_file, 'Sheet', 'QuarterlyData', 'Range', 'A4:E500', 'ReadVariableNames', true);
names_q = data_quarterly.Properties.VariableNames;

data_monthly = readtable(dat_file, 'Sheet', 'MonthlyData', 'Range', 'A4:AQ2000', 'ReadVariableNames', true);
names_m = data_monthly.Properties.VariableNames;

%% 1) select publication delay and publication grouping
%{
% Publication Delay
...
%% 3) define data sets and names

% Use indices to select the correct data and time vectors (checked)
DateString = '29-02-1960';
formatIn = 'dd-mm-yyyy';

tbl = data_monthly(2:find(data_monthly{:,1} == end_s, 1, 'first'),data_incl_m+1);

data_m = tableToNumericMatrix(tbl);
transf = tableToNumericMatrix(data_monthly(1,data_incl_m+1));        %transforms for monthly data defined directly in excel
[T,n] = size(data_m);
varnames = names_m(1,[data_incl_m+1]);
d_m = table2array(data_monthly(2:find(data_monthly{:,1} == end_s, 1, 'first'),1)); % date names

tbl = data_quarterly(2:find(data_quarterly{:,1} == end_s, 1, 'first'),data_incl_q+1);
data_q = tableToNumericMatrix(tbl);
[Tq,n_q] = size(data_q);
varnames_q = names_q(1,[data_incl_q+1]);
d_q = table2array(data_quarterly(2:find(data_quarterly{:,1} == end_s, 1, 'first'),1));
...
if act==1 && sur==1 && act==1&&lab==1 && mort==1
...
clear tbl data_monthly data_quarterly names_m names_q Var Varq Varm Varp Varl Vara Vari Varv Varf Vars Vara_delay Vara_pubgroup Varf_pubgroup Varf_delay Vari_pubgroup Vari_delay Varm_pubgroup Varm_delay Varp_delay Varp_pubgroup Varq_delay Varq_pubgroup Vars_pubgroup Vars_delay Varv_pubgroup Varv_delay Varl_delay Varl_pubgroup Varmt_pubgroup Varmt_delay 

%%  4) Transform data (checked)
...
%% 5) Select, Transform and plot data

if dyoy ==1 
    d_m = d_m(12:end,:);
end
t1_m = find(d_m==beg_s)-12; % This will change as a test, was at -2
t1_q = find(d_q==beg_s)+1;
tend_m = find(year(d_m)==year(end_s),1,'last');
tend_q = find(d_q==end_s);

y_q = data_q_tr(t1_q:tend_q,:); 
y = data_m_tr(t1_m:tend_m-1,:); %% Check that this is correct!!
T = length(y);
Tq = length(y_q);
d_m = d_m(t1_m:tend_m);
d_q = d_q(t1_q:tend_q);
...
y_m = y;
varnames_qm = [varnames(1,1:end),varnames_q(1,1:end)];
