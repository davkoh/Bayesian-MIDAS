...
function [h_tilde,h0,omegah,omegah_hat,Domegah,Vbeta,deltabeta,lambdabeta] = ...
    SVRW_gam_dgamm(ystar,h_tilde,h0,omegah,a0,Vbeta,deltabeta,lambdabeta)
...
% Define a few things
    ...
    %% sample Vbeta
    for i = 1:2
          Vbeta(i) = gigrnd(deltabeta - 0.5,deltabeta*lambdabeta,beta(i)^2 ,1);
    end
    Vbeta = Vbeta + 10^(-10);
    sumbeta1 = sum(Vbeta(:));
    sumbeta2 = sum(log(Vbeta(:)));    
    %% sample lambdabeta
    lambdabeta = gamrnd( 0.01 + 2*deltabeta, 1/(0.01 + 0.5*deltabeta*sumbeta1  ));   
    %% sample deltabeta
    logNewdeltabeta = log(deltabeta) + 0.01*randn;
    Newdeltabeta = exp( logNewdeltabeta ); 
    loglikebeta = @(x) -2*gammaln(x) + 2*x*log(0.5*x*lambdabeta) + (x-1)*sumbeta2 - 0.5*x*lambdabeta*sumbeta1;
    logalp = loglikebeta(Newdeltabeta) - loglikebeta(deltabeta) + (deltabeta - Newdeltabeta) + log(Newdeltabeta) - log(deltabeta); 
    if log(rand) < logalp
        deltabeta = Newdeltabeta;
        %countdeltabeta = countdeltabeta + 1;
    end     
