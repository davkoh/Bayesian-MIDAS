%% GGIG Regression Model Gibbs Sampler: Corrected for erros in trend and t-errors %%
...
[h,iOh,h0,omegah] = h_samp(Y,tau,X,beta,lam,h_tilde,h0,omegah,a0_h,b0_h,Vomegah,T,sv_ind);
...
[g,g0,omegag] = g_samp(tau,tau0,g_tilde,g0,omegag,a0_g,b0_g,Vomegag);
...
[lam,nu_y] =  t_samp(Y,tau,X,beta,h,nu_y,nu_lb,nu_ub,alpha_nu,beta_nu,lam,T);
else
    lam = ones(T,1);
    nu_y = 1;
...
%% Sample h
function [h,iOh,h0,omegah] = h_samp(y,tau,X,beta,lam,h_tilde,h0,omegah,a0_h,b0_h,Vomegah,T,sv_ind)
...
%% Sample g
function [g,g0,omegag] = g_samp(tau,tau0,g_tilde,g0,omegag,a0_g,b0_g,Vomegag)
...
function [lam,nu_y] =  t_samp(Y,tau,X,beta,h,nu_y,nu_lb,nu_ub,alpha_nu,beta_nu,lam,T) 
...
%% sample lam
lam = 1./gamrnd((nu_y+1)/2,2./(nu_y+(1./(exp(h).*lam).*speye(T))*((Y-tau-X*beta).^2)));
%
nu_y = sample_nu_slice(nu_y,lam,nu_lb,nu_ub,alpha_nu,beta_nu);
