...
for i = 1:iter
    %% 1: Sample Dynamic Shrinkage Components
        ...
        Ch = chol(Kh,'lower');
        htilde_hat = Kh\(Hphi'*iSig_xi*Hphi*ones(T,1)*h_tilde0  + iSig_v*(ystar- m_s -mu_tilde));
        ...
        % (c) Sample mu
        Kmu = xi_mu + xi_0 + (1-phi^2)*sum(xi(1:T));
        Cm = chol(Kmu,'lower');
        mu_hat = Kmu\(( xi_0*h0 + (1-phi)*sum(xi(1:T).*[h(1)-phi*h0;h(2:T)-phi*h(1:T-1)])));
        mu = mu_hat + Cm'\randn(1,1);
        mu_tilde = [(1-phi)*ones(T,1)*mu];

        % (d) Sample xi_mu
        xi_mu = 1./pgdraw(mu);

        % (e) Sample h_tilde0
        Khtilde0 = (xi_0+ phi^2*xi(1));
        Chtile0 = chol(Khtilde0,'lower');
        htilde0_hat = Khtilde0\(xi(1)*phi*h_tilde(1) + xi_0*mu);
        h_tilde0 = htilde0_hat + Chtile0'\randn(1,1);
        h0 = mu + h_tilde0;

        % (e) Sample xi_0 
        xi_0 = 1./pgdraw(h_tilde0);

        % (f) Sample phi (needs slice sampling)
            % define y_ar
            y_ar = h_tilde(1:end)./(xi(1:end).^(1/2));
            x_ar = [h_tilde0;h_tilde(1:end-1)]./([xi_0;xi(1:end-1).^(1/2)]);
    ...
    %% 2: Sample State variables
        ...
        % (b) tau0
        Ktau0 = (1/exp(h(1)) + 1/exp(h0));
        Ctau0 = chol(Ktau0,'lower');
        tau0_hat = Ktau0\(tau(1)/exp(h(1)));
        tau0 = tau0_hat + Ctau0'\randn(1,1);
