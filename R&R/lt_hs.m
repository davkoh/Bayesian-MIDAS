%% The Local Trend Model with Horseshoe Prior %%
...
for i = 1:iter
    % Sample \tau
    K_tau = H'*sparse(diag(1./Sigma_tau))*H + 1/sig2*speye(T);
    C_tau = chol(K_tau,"lower");
    tau_hat = K_tau\(tau0*H'*sparse(diag(1./Sigma_tau))*H*ones(T,1) + y/sig2);
