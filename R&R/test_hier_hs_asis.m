%% Testing the Non-centred hierarchical horseshoe prior Model (with interweaving) %%
...
for i = 1:iter
    ...
    %% Interweaving step
    % Transform to Centred State process
    tau_c = alpha + omegatau*tau;
    tau0_c = alpha + tautilde_0*omegatau;

    % Update alpha and omegatau^2
    omegatau2 = gigrnd(-1/2,1/Sigma_tau(2),sum((tau_c - [tau0_c;tau_c(1:T-1)]).^2) + (tau0_c - alpha)^2/V_tautilde0,1);
    alpha = (tau0_c*Sigma_tau(1))/(Sigma_tau(1) + omegatau2*V_tautilde0) + sqrt(omegatau2*V_tautilde0*Sigma_tau(1)/(omegatau2*V_tautilde0*Sigma_tau(1) + Sigma_tau(1)));
    omegatau = sqrt(omegatau2)*U;
    
    % Update tau
    tau = (tau_c - alpha)./(omegatau);
    tautilde_0 = (tau0_c - alpha)/omegatau;
    ...
    tau0_save(isave) = alpha + omegatau*tautilde_0;
