...
function out = bmidas_gigg_dev(input)
...
if strcmp(sv_obs_type,"DHS")
phi = 0; % Start at zero perhaps
phi_trend = 0; % Start at zero perhaps
Hphi = speye(T) - sparse(2:T,1:(T-1),phi*ones(1,T-1),T,T);
Hphi_trend = speye(T) - sparse(2:T,1:(T-1),phi*ones(1,T-1),T,T);
H = speye(T) - sparse(2:T,1:(T-1),ones(1,T-1),T,T);
mu = 0;
mu_trend = [mu;(1-phi)*ones(T-1,1)*mu];
...
iSig_xi_trend = sparse(1:T,1:T,1./xi);
xi_0 = 0;
xi_0_trend = 0;
xi_mu = 0;
xi_mu_trend = 0;sig2_y = 1;
lamt = ones(T,1);
nut = 5;
alpha_nu = 2;
beta_nu = 1;
...
if strcmp(sv_obs_type,"DHS")
    % normal mixture
        ...
        temprand = rand(T,1);
        q_SV = repmat(pj,T,1).*normpdf(repmat(ystar,1,10),...
        repmat(h_tilde,1,10)+repmat(mj,T,1),repmat(sigj,T,1));
        q_SV = q_SV./repmat(sum(q_SV,2),1,10);
        S = 10 - sum(repmat(temprand,1,10)<cumsum(q_SV,2),2)+1;
        m_s = mj(S)'; iSig_v = sparse(1:T,1:T,1./sigj2(S));

        Kg= (iSig_v + Hphi'*iSig_xi*Hphi);
        Cg = chol(Kg,'lower');
        htilde_hat = Kg\(iSig_v*(ystar- m_s -mu_tilde));
        h_tilde = htilde_hat + Cg'\randn(T,1);
        h = h_tilde + mu;
        ...
        % (c) Sample mu, global ~ C(0,1/sqrt(T)\nu_global), local ~ Cauchy(0,1)
        Kmu = xi_mu + xi_0 + (1-phi^2)*sum(xi(2:T));
        Cm = chol(Kmu,'lower');
        mu_hat = Kmu\(( xi_0*h(1) + (1-phi)*sum(xi(2:T).*[h(2:T)-phi*h(1:T-1)])));
